...
function theme_save_features_postdata($post_id) 
{
	...
	if (!wp_verify_nonce($_POST[$themename . '_features_noncename'], plugin_basename( __FILE__ )))
		return;
...
function theme_features_shortcode($atts)
{
	...
	if(have_posts())
	{
		...
		$output .= '<li class="item_content clearfix">
				<a class="features_image" href="' . get_permalink() . '" title="' . esc_attr(get_the_title()) . '"><img src="' . get_template_directory_uri() . '/images/icons_' . $type . '/' . get_post_meta(get_the_ID(), "icon_color", true) . '/' . get_post_meta(get_the_ID(), "icon", true) . '.png" /></a><div class="text">'
				. ((int)$headers==1 ? '<h3><a href="' . get_permalink() . '" title="' . esc_attr(get_the_title()) . '">' . get_the_title() . '</a></h3>' : '')
