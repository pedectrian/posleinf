...
function theme_gallery_shortcode($atts)
{
	...
	$args = array( 
		...
		'orderby' => (count(explode(",", $order_by))>1 ? "'" : "") . implode(" ", explode(",", $order_by)) . (count(explode(",", $order_by))>1 ? "'" : ""), 
		'order' => $order
	);
	...
	if(have_posts())
	{
		...
		if($type=="list_with_details" || $type=="details")
		{
			...
			$output .= '<li id="gallery-details-' . get_the_ID() . '" class="gallery_item_details clearfix">
					...
					if((int)$display_headers)
						$output .= '<' . $headers_type . ' class="box_header"> ' . get_the_title() . '</' . $headers_type . '>';
		...
		if($type!="details")
		{
			...
			if($display_method=="dm_carousel")
				...
				<div class="header_left">' . ($header!="" ? '<h3 class="box_header">' . $header . '</h3>' : '') . '</div>
			...
			if($display_method=="dm_filters")
				...
				$output .= '<li class="gallery_box" id="gallery-item-' . get_the_ID() . '">';
				if(has_post_thumbnail())
				...
				$output .= '
						<ul class="controls">
							<li>
								<a href="' . ($type=="list" && ((int)$details_page) ? get_permalink((int)$details_page) : '') . '#gallery-details-' . get_the_ID() . '" class="open_details" style="background-image: url(\'' . get_template_directory_uri() . '/images/icons_media/' . $lightbox_icon_color . '/details.png\')"></a>
							</li>';
				if(has_post_thumbnail())
				{
					$output .= '<li>';
