...
{
	//dropdownmulti
	add_shortcode_param('dropdownmulti' , 'dropdownmultiple_settings_field');
	...
	//hidden
	add_shortcode_param('hidden', 'hidden_settings_field');
	...
	//add item button
	add_shortcode_param('listitem' , 'listitem_settings_field');
	function listitem_settings_field($settings, $value)
	{
		$dependency = vc_generate_dependencies_attributes($settings);
		$value = explode(",", $value);
		$output = '<input type="button" value="' . __('Add list item', 'medicenter') . '" name="'.$settings['param_name'].'" class="button '.$settings['param_name'].' '.$settings['type'].'" style="width: auto; padding: 0 10px 1px;"' . $dependency . '/>';
		return $output;
	}
	//add item window
	add_shortcode_param('listitemwindow' , 'listitemwindow_settings_field');
	function listitemwindow_settings_field($settings, $value)
	{
		$value = explode(",", $value);
		$output = '<div class="listitemwindow modal" name="'.$settings['param_name'].'">
			<div class="modal-header">
				<button class="close cancel-item-options" type="button">×</button>
				<h3>' . __('Add New List Item', 'medicenter') . '</h3>
			</div>
