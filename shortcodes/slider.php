...
function theme_slider($atts)
{
	...
	if($slider_options)
	{
		...
		{
			$output .= '<ul class="slider ' . $id . ' id-' . $id . ' autoplay-' . $slider_options['slider_autoplay'] . ' pause_on_hover-' . $slider_options['slider_pause_on_hover'] .  ' interval-' . $slider_options['slide_interval'] . ' effect-' . $slider_options['slider_effect'] . ' easing-' . $slider_options['slider_transition'] . ' duration-' . $slider_options['slider_transition_speed'] . /*((int)$ontouch ? ' ontouch' : '') . ((int)$onmouse ? ' onmouse' : '') .*/ '">';
...
function theme_slider_content($atts)
{
	...
	if($slider_options)
	{
		...
		if(count($slides_count))
		{
			...
			for($i=0; $i<$slides_count; $i++)
			{
				if($slider_options["slider_image_title"][$i]!="" || $slider_options["slider_image_subtitle"][$i]!="")
				{
					...
					if($slider_options["slider_image_title"][$i]!="")
						$output .= '<h1 class="title">' . $slider_options["slider_image_title"][$i] . '</h1>';
					if($slider_options["slider_image_subtitle"][$i]!="")
