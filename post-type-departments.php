...
function theme_departments_init()
{
	...
	if(!get_option($themename . "_department_hours_table_installed"))
	{
		...
		$query = "CREATE TABLE IF NOT EXISTS `wp_" . $blog_id . "_department_hours` (
		...
		$wpdb->query($query);
		//insert sample data
		$query = "INSERT INTO `wp_" . $blog_id . "_department_hours` (`department_hours_id`, `department_id`, `weekday_id`, `start`, `end`, `tooltip`, `before_hour_text`, `after_hour_text`, `doctors`, `category`) VALUES
...
function theme_get_department_hour_details()
{
	global $blog_id;
	global $wpdb;
	$query = "SELECT * FROM `wp_" . $blog_id . "_department_hours` AS t1 LEFT JOIN {$wpdb->posts} AS t2 ON t1.weekday_id=t2.ID WHERE t1.department_id='" . $_POST["post_id"] . "' AND t1.department_hours_id='" . $_POST["id"] . "'";
...
function theme_inner_departments_custom_box_side($post) 
{
	...
	$query = "SELECT * FROM `wp_" . $blog_id . "_department_hours` AS t1 LEFT JOIN {$wpdb->posts} AS t2 ON t1.weekday_id=t2.ID WHERE t1.department_id='" . $post->ID . "' ORDER BY FIELD(t2.menu_order,2,3,4,5,6,7,1), t1.start, t1.end";
...
function theme_save_departments_postdata($post_id) 
{
	...
	if (!wp_verify_nonce($_POST[$themename . '_departments_noncename'], plugin_basename( __FILE__ )))
		return;
	...
	{
		$query = "INSERT INTO `wp_" . $blog_id . "_department_hours` VALUES(
			NULL,
			'" . $post_id . "',
			'" . $_POST["weekday_ids"][$i] . "',
			'" . $_POST["start_hours"][$i] . "',
	...
	$delete_department_hours_ids_count = count($_POST["delete_department_hours_ids"]);
	if($delete_department_hours_ids_count)
		$wpdb->query("DELETE FROM `wp_" . $blog_id . "_department_hours` WHERE department_hours_id IN(" . implode(",", $_POST["delete_department_hours_ids"]) . ");");
	//post meta
